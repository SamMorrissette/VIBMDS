...
#' @import torch
#' @examples
compute_log_joint <- function(dist_mat, z, prior_params, B) {
  log_lik <- log_lik_minibatch(dist_mat, z, B)
  ...
  p_x <- distr_normal(loc = 0, scale = z$lambda$unsqueeze(2))$log_prob(z$x)$sum(dim=c(2,3))
