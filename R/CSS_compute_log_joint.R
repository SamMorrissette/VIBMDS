...
CSS_compute_log_joint <- function(dist_vec, z, prior_params, B, device) {
  ...
  # p_omega <- torch_log_beta(z$omega$squeeze(), prior_params$a_omega, prior_params$b_omega)

  #alpha_omega <- z$omega / (1 - z$omega)
  #p_z_gates <- torch_log_binconcrete(z$z_gates, alpha_omega, prior_params$temp)$sum(dim = 2)

  p_z_gates <- torch_log_binconcrete(z$z_gates, prior_params$loc, prior_params$temp)$sum(dim = 2)
  log_lik + p_sigma + p_lambda + p_x_tilde + p_z_gates #+ p_omega
}
